...
export type HostExtensionStep = CompositionStep;
...
export async function update(target: Image[]) {
	...
	if (target.length > 0) {
		...
		target.forEach((image) => args.push([image.appUuid, image.name].join(':')));
	}
...
function extensionImagesToInstall(
	targetState: Dictionary<App>,
	installedExtensions: HostExtensionContainer[],
): Service[] {
